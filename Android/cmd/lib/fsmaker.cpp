...
FsMaker *FsMaker::create(char *fname)
{
	...
	if (!fsmaker) {
		::fprintf(stderr, "can not new FsMaker Instance.\n");
		return NULL;
	}
	fsmaker->dirmgr = NULL;
	fsmaker->iexmgr = NULL;
	...
	if (err < 0) {
		::fprintf(stderr, "fstat(2) %s err=%d\n", fsmaker->fname, errno);
		delete fsmaker;
		return NULL;
	}
	...
	TARFS_IO(err, TARFS_IO_R, fsmaker->fd, &fsmaker->sb, 
				TAR_BLOCKSIZE, off*TAR_BLOCKSIZE);
	if (err != 0){
		::fprintf(stderr, "read err %s off=%Lu err=%d\n", fsmaker->fname, off, err);
		delete fsmaker;
		return NULL;
	}
	if (fsmaker->sb.tarfs_magic == TARSBLOCK_MAGIC) {
		/* already completed mkfs_tar */
		delete fsmaker;
		return NULL;
	}
	// super block
	...
	if (!fsmaker->dirmgr || !fsmaker->iexmgr || !fsmaker->dinodemgr) {
		::fprintf(stderr, "can not new SpaceManger Instance.\n");
		delete fsmaker;
		return NULL;
	}
	// free dinode
	...
	if (ret == false) {
		::fprintf(stderr, "InodeFactory init failed.\n");
		delete fsmaker;
		return NULL;
	}
	...
	if (inode == NULL) {
		::fprintf(stderr, "InodeFactory::allocInode failed.\n");
		delete fsmaker;
		return NULL;
	}
...
{
	int err = 0;
	TARFS_IO(err, TARFS_IO_R, this->fd, bufp, TAR_BLOCKSIZE*nblks, 
				blkno*(TARBLK)TAR_BLOCKSIZE);
	if (err != 0) {
		::fprintf(stderr, "readBlock:blkno=%lld err = %d\n", blkno, err);
		this->rollback();
	}
	return;
}
void FsMaker::writeBlock(char *bufp, TARBLK blkno, ssize_t nblks)
{
	...
	TARFS_IO(err, TARFS_IO_W, this->fd, bufp, TAR_BLOCKSIZE*nblks, 
				blkno*(TARBLK)TAR_BLOCKSIZE);
	if (err != 0) {
		::fprintf(stderr, "writeBlock:blkno=%lld err = %d\n", blkno, err);
		this->rollback();
	}
	return;
}
